...
public class AsBrowserHoverer extends AgentScopeSandboxAwareTool<HoverTool> {
    public AsBrowserHoverer() {
        super(new HoverTool());
    }
    ...
    public Mono<ToolResultBlock> callAsync(Map<String, Object> input) {
        ...
        try {
            String result = sandboxTool.browser_hover(element, ref);
            return Mono.just(ToolResultBlock.text(result));
        } catch (Exception e) {
            return Mono.just(ToolResultBlock.error(
                    "Browser hover error: " + e.getMessage()));
        }
    }
}
